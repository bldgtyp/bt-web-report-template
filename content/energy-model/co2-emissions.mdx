...
Output Emission Rates used are from the CO (RMPA) Subregion. For more information on these factors see the EPA eGRID Data Explorer and Guidance on the Use of eGRID Output Emission Rates. Source Energy Factors for all fuel types are taken from the EPA EnergyStar Portfolio Manager Technical Reference (2018).

Note: While CO2e emissions are calculated within the Passive House energy modeling software (PHPP), CO2e is not one of the certification metrics for Passive House projects. The following is thus informational only.
